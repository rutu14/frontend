...
export const login = ( email,password ) => async (dispatch) => {
  try {
      ...
      const { data } = await axios.post( baseURL + 'signIn',{ email,password, role:'Customer' },config)
...
export const register = (email, password) => async (dispatch) => {
  try {
	...
	const config = { headers: { 'Content-Type': 'application/json' } }
    const { data } = await axios.post( baseURL + 'createUser',{ email, password,role:'Customer' },config )
